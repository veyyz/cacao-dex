...
contract("Zrx", async (accounts) => {
  let zrx;

  beforeEach(async () => {
    zrx = await Zrx.deployed();
  });

  it("should deploy to the network and have an address", async () => {
    assert(
      web3.utils.isAddress(zrx.address),
  ...
  it("should return token name, symbol, and decimals", async () => {
    ...
    const decimals = await zrx.decimals();

    assert(name === "0x Protocol Token", "Name not set properly");
    assert(symbol === "ZRX", "Symbol not set properly");
    assert(decimals == 18, "Decimals not set properly");
  });

  it("should mint new tokens and send to a given address", async () => {
    await zrx.faucet(accounts[0], "100000");
    const balance = await zrx.balanceOf(accounts[0]);
    // returns BN {
    //   negative: 0,
  ...
  it("should allow an account to transfer its balance to another account", async () => {
    const _balance0 = await zrx.balanceOf(accounts[0]);
    const _balance1 = await zrx.balanceOf(accounts[1]);

    await zrx.transfer(accounts[1], _balance0.toString(), {
      from: accounts[0],
    });

    const balance0 = await zrx.balanceOf(accounts[0]);
    const balance1 = await zrx.balanceOf(accounts[1]);
  ...
  it("should allow an account to approve another account to spend its balance", async () => {
    ...
    await zrx.approve(accounts[0], _balance1.toString(), { from: accounts[1] });
    await zrx.transferFrom(accounts[1], accounts[0], _balance1.toString(), {
      from: accounts[0],
    });
  ...
  it("should NOT allow an account to spend another accounts balance without approval", async () => {
    ...
    await expectRevert(
      zrx.transferFrom(accounts[0], accounts[1], _balance0.toString(), {
        from: accounts[1],
      }),
